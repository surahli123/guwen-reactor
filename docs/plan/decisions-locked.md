...
5. **Unsupported asserted event (structural "cover" check — fix C1 + 2026-06-28 review #4):** an `action`/`visual` claim → `UNSUPPORTED_DETAIL` flagged **critical** → blocks when its `source_fact_ids` do **not structurally cover** it. "Cover" is **deterministic, NOT semantic**: flag if (a) `source_fact_ids` is empty, OR (b) any cited id does **not belong to the claim's declared `beat_id`** fact-set in `canon_gold` (a cross-beat / valid-but-irrelevant citation), OR (c) the claim is beat-less (`beat_id=None`) yet cites an id belonging to *some* beat — grounded in a beat-owned fact but refusing to declare its beat (eval-core review). A genuinely non-beat fact (e.g. `F10`) is still allowed beat-less. This closes the "fabricate an event + cite a real-but-wrong valid id" hole, and such an irrelevant id must **not** be added to `supported_fact_ids` (no coverage boost). Semantic entailment of `claim_text` by the fact stays **advisory-only** (D1) — **disclosed residual:** a fabricated sentence citing a *correct-beat* id is not caught deterministically. Counting unchanged (UNSUPPORTED_DETAIL → `unsupported_critical_claims`). **Required hostile fixture:** an `evals/test_structural_audit.py` case with a valid-but-wrong-beat id → BLOCKED.
